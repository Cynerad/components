...
function FieldTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="field-label"
      className={cn("flex w-fit items-center gap-2 text-xs/relaxed font-medium group-data-[disabled=true]/field:opacity-50", className)}
      {...props}
    />
  );
}
...
function FieldError({
  ...
  return (
    <div role="alert" data-slot="field-error" className={cn("text-xs/relaxed font-normal text-destructive", className)} {...props}>
      {content}
    </div>
  );
}
