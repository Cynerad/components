...
function StatLabel({ className, ...props }: ComponentProps<"div">) {
  return <div data-slot="stat-label" className={cn("font-medium text-muted-foreground text-sm", className)} {...props} />;
}

const statIndicatorVariants = cva("flex shrink-0 items-center justify-center [&_svg]:pointer-events-none", {
  variants: {
    variant: {
...
type StatIndicatorType = Omit<ComponentProps<"div">, "color"> & VariantProps<typeof statIndicatorVariants>;

function StatIndicator({ className, variant = "default", color = "default", ...props }: StatIndicatorType) {
  return (
    <div
...
function StatValue({ className, ...props }: ComponentProps<"div">) {
  return <div data-slot="stat-value" className={cn("font-semibold text-2xl tracking-tight", className)} {...props} />;
}
...
function StatDescription({ className, ...props }: ComponentProps<"div">) {
  return <div data-slot="stat-description" className={cn("text-muted-foreground text-xs", className)} {...props} />;
}
